...
use Planillas\CoreBundle\Entity\CEmpleadoBaseDatosDigital;
use Planillas\CoreBundle\Form\Type\CEmpleadoBaseDatosDigitalType;
use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use Symfony\Component\HttpFoundation\Request;

class CEmpleadoBaseDatosDigitalController extends Controller
{
    ...
    public function newAction($id_empleado, Request $request)
    {
        ...
        $entity = new CEmpleadoBaseDatosDigital();
        $form = $this->createForm(new CEmpleadoBaseDatosDigitalType(), $entity, array(
            'method' => 'POST',
            'action' => $this->generateUrl('empleado_basedatos_digital_new', array('id_empleado' => $id_empleado)),
        ));

        $form->handleRequest($request);
        if($form->isSubmitted() && $form->isValid()) {
            if ($form->get('cancelar')->isClicked()) {
                return $this->redirect($this->generateUrl('empleado_basedatos_digital', array(
        ...
        return $this->render('PlanillasCoreBundle:CEmpleadoBaseDatosDigital:new.html.twig', array(
    ...
    public function deleteAction($id_empleado, Request $request)
    {
        ...
        if($form->isValid()) {
            ...
            $archivo_bddigital = $em->getRepository('PlanillasCoreBundle:CEmpleadoBaseDatosDigital')->find($data['id']);
        ...
        return $this->redirect($this->generateUrl('empleado_basedatos_digital', array('id_empleado' => $id_empleado)));
    }
    ...
    private function createDeleteFormReferencias($id_empleado, $id=null, $csrf=false)
    {
        $form = $this->get('form.factory')->createNamedBuilder('delete_form', 'form', array('id' => $id), array(
            ...
            ->add('id','hidden',array())
            ->setMethod('DELETE')
            ->setAction($this->generateUrl('empleado_basedatos_digital_delete', array('id_empleado' => $id_empleado)))
            ->getForm();
